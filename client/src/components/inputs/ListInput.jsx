...
export default function ListInput({ setterFunction, listTitle, setIsAdding, addList}) {
    return (
        <span className={"flex justify-between items-center gap-1 mx-1"}>
            <input type={"text"} className={"text-gray-500 w-full bg-offwhite text-lg p-2 rounded-xl border-none outline-none"} onChange={(e) => {setterFunction(e.target.value)}} maxLength={16}/>
